...
;
; 1/19/2010 - jdm - initial version
;
;*****************

DROP PROGRAM   Z_JDM_RX_ORDER_ACTION_USERS : DBA  GO
CREATE PROGRAM  Z_JDM_RX_ORDER_ACTION_USERS : DBA 
prompt 
	...
	, "Choose the following actions:" = "All Available" 

with OUTDEV, STARTDATE, STOPDATE, FACILITY, USERID, DISPLAY 
...
SET  LINE1  =  FILLSTRING (170 , "-" )

SET  LINE2  =  FILLSTRING (170 , "=" )
...
IF ( ( CNVTUPPER ( TRIM ( $OUTDEV ))!="MINE" ) ) 
 WITH  DIO = POSTSCRIPT , MAXROW =45 , MAXCOL =180 , COUNTER , FORMAT , FORMAT = VARIABLE ,
 NULLREPORT 
ELSE  
ENDIF
 INTO  $OUTDEV 
...
ENDIF
,
 ACTIONPERSONID = EVALUATE (OA.NEEDS_VERIFY_IND, 3 , OA.UPDT_ID, OA.ACTION_PERSONNEL_ID),
 NURSE_UNIT = UAR_GET_CODE_DESCRIPTION (E.LOC_NURSE_UNIT_CD),
 FACILITY_AREA = UAR_GET_CODE_DISPLAY (E.LOC_FACILITY_CD),
 USER_NAME_DISPLAY =P.NAME_FULL_FORMATTED
...
( DUMMYT  D  WITH  SEQ = VALUE (( NEXPANDTOTAL / NEXPANDSIZE )))
 PLAN ( OA 
WHERE ( CNVTSTRING ( EVALUATE (OA.NEEDS_VERIFY_IND, 3 , OA.UPDT_ID, OA.ACTION_PERSONNEL_ID))=
 $USERID ) AND (OA.ACTION_DT_TM BETWEEN  CNVTDATETIME ( INTERNAL -> BEGIN_DT_TM ) AND  CNVTDATETIME 
...
,
 COL 01 ,
"Z_JDM_RX_ORDER_ACTION_USERS" ,
 CALL CENTER ("ORDER ACTIONS BY USER" , 1 , 160 ),
 COL 161 ,
"Page: " ,
 CURPAGE "###" 
,
...
HEAD  ACTIONPERSONID 
 ...
 CALL ECHO ( BUILD ("Head action prsnl id --" , OA.ACTION_PERSONNEL_ID)),
FOR (  X  = 1  TO  NACTION_TYPE_CNT  )
 STAT = ALTERLIST ( ACTION_TYPES -> TYPE [ X ]-> PERSON_HOUR , 0 ), STAT = ALTERLIST ( ACTION_TYPES 
-> TYPE [ X ]-> PERSON_HOUR , 24 )

ENDFOR
,
IF ( ( ROW >=36 ) ) 
 CALL ECHO ("Breaking in head action personnel" ), BREAK
ENDIF
,
IF ( ( BNEW_PAGE =1 ) ) 
IF ( (E.LOC_FACILITY_CD=0.0 ) )  COL 01 , "Facility: " , "Unknown" 
...
,  ROW +2 
ENDIF
, COL 01 ,"User Name: " , USER_NAME_DISPLAY , ROW +1 , COL 20 ,
...
IF ( ( NSKIPDETAIL =0 ) ) 
 CALL ECHO ( BUILD ("Detail/Order id --" , OA.ORDER_ID)), 
 CALL ECHO ( BUILD ("facility_area " ,  FACILITY_AREA )),  BNEW_PAGE =0 , 
...
IF ( ( W >0 ) )  AHOUR [ IDX ]=( AHOUR [ IDX ]+1 ),  ATOTAL [ IDX ]=( ATOTAL [ IDX ]+1 ),  AGRDTOT [
 IDX ]=( AGRDTOT [ IDX ]+1 ),  ACTION_TYPES -> TYPE [ W ]-> PERSON_HOUR [ IDX ]-> TOTAL =(
 ACTION_TYPES -> TYPE [ W ]-> PERSON_HOUR [ IDX ]-> TOTAL +1 ),  ACTION_TYPES -> TYPE [ W ]->
...
IF ( ( BALL_ACTIONS =0 ) )  ACT_TYPE_OUTPUT = FILLSTRING (18 , " " ),  ACT_TYPE_OUTPUT = SUBSTRING (
1 , 18 ,  ACT_TYPE ),  COL 01 ,  ACT_TYPE_OUTPUT ,  COL 20 , 
FOR (  X  = 1  TO 24  )
...
FOOT   ACTIONPERSONID 
 ...
 CALL ECHO ("Foot action personnel" ),
IF ( ( BALL_ACTIONS =1 ) ) 
FOR (  Y  = 1  TO  NACTION_TYPE_CNT  )
 COL 01 , ACTION_TYPES -> TYPE [ Y ]-> DISPLAY , COL 20 ,
FOR (  X  = 1  TO 24  )

IF ( ( ACTION_TYPES -> TYPE [ Y ]-> PERSON_HOUR [ X ]-> TOTAL >0 ) )  ACTION_TYPES -> TYPE [ Y ]->
 PERSON_HOUR [ X ]-> TOTAL "#####" 
,  DT =( DT + ACTION_TYPES -> TYPE [ Y ]-> PERSON_HOUR [ X ]-> TOTAL )
ELSE  "-----" 
ENDIF
, COL +1 
...
,  DT "######" 
,  DT =0 ,  ROW +1 , 
IF ( ( BNEW_PAGE =1 ) )  BNEW_PAGE =0 ,  COL 01 ,  LINE1 ,  ROW +1 , 
IF ( (E.LOC_FACILITY_CD=0.0 ) )  COL 01 , "Total Orders: " , "Unknown" 
ELSE   COL 01 , "Total Orders: " ,  FACILITY_AREA 
ENDIF
,  ROW +1 ,  ROW +1 ,  COL 20 , 
...
,  DT "######" 
,  DT =0 ,  ROW +1 , 
IF ( ( BNEW_PAGE =1 ) )  BNEW_PAGE =0 ,  COL 01 ,  LINE1 ,  ROW +1 ,  COL 01 , 
"Total Orders: All Facilities " ,  ROW +1 ,  COL 20 , 
" 0000  0100  0200  0300  0400  0500  0600  0700  0800  0900  1000  1100  1200  1300 " , 
" 1400  1500  1600  1700  1800  1900  2000  2100  2200  2300 Total" ,  ROW +1 ,  COL 01 , 
"Action Type: " ,  COL 20 , 
" 0059  0159  0259  0359  0459  0559  0659  0759  0859  0959  1059  1159  1259  1359 " , 
...
ENDIF
 ...
 CALL ECHO ("Last_Mod = 016 & Mod_Date = 01/10/2008" )

 CALL ECHO ("Last Mod = 019" )

 CALL ECHO ("Mod Date = 11/19/2010" )
 END GO
