...
;===============================================================================
; Program: 1_cust_mp_mob_get_pdata_16.prg
; Version: v16
; Build Date: 2026-01-13
; Project: Mobility Dashboard v2.12.0-mobility
; Issue: #32 - Fix Duplicate Activity Orders
;
; Features: demographics, patient-list-integration, 30-day-lookback, 14-clinical-events, activity-orders, activity-precautions, historical-arrays, bmat-parsing, baseline-mobility, toileting-method, pt-ot-transfers, ambulation-distance, ambulation-personnel, powerform-discrete-grid, order-detection, powerform-activity-links, transfer-type, position-activity, order-deduplication
;
...
;
; Changelog v16:
;   - FIXED: Duplicate orders showing all scheduled instances (Issue #32)
;   - CHANGED: SELECT 4 (Precautions) uses HEAD o.order_mnemonic for deduplication
;   - CHANGED: SELECT 7 (Activity Orders) uses HEAD o.order_mnemonic for deduplication
;   - Result: Shows unique order types only (e.g., 2 orders instead of 279)
;   - Based on v15 with order deduplication fix for both columns
;
; Changelog v15:
...
DECLARE PROGRAM_VERSION = vc WITH CONSTANT("v16"), PROTECT
...
ORDER BY o.encntr_id, o.order_mnemonic, o.orig_order_dt_tm DESC
...
HEAD o.order_mnemonic
	; New unique precaution type - add to array (only first/most recent instance)
	; Issue #32: Deduplicates recurring precaution orders for consistency
...
HEAD o.order_mnemonic
	; New unique order type - add to array (only first/most recent instance)
	; Issue #32: Deduplicates recurring orders (e.g., daily "Up to Chair" becomes 1 entry)
