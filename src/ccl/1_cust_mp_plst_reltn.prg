...
DROP PROGRAM   1_cust_mp_plst_reltn : DBA  GO
CREATE PROGRAM  1_cust_mp_plst_reltn : DBA
...
DECLARE  EXECUTEEPRSELECT (( X = I4 )) =  NULL
 
DECLARE  EXECUTEPPRSELECT (( X = I4 )) =  NULL
 
DECLARE  PARSEENCNTRLOCHIST (( X = I4 )) =  NULL
 
DECLARE  EXAMINEPROVIDER (( X = I4 )) =  NULL
 
DECLARE  EXAMINEUSER (( X = I4 )) =  NULL
...
IF ( ( CRITERIA -> TIME_FRAME_FLAG != NOTIMEPARAMETER ) AND  (( ( CRITERIA -> LOCATIONS -> UNIT_CNT
>0 ) )  OR  ( (( ( CRITERIA -> SERVICE_CNT >0 ) )  OR  (( CRITERIA -> TYPE_CNT >0 ) ))  ))  )
SET  USEELH  = 1
ENDIF
...
IF ( ( CRITERIA -> VRELTN_CNT >0 ) AND ( CRITERIA -> RELTN_PRSNL_ID = REQINFO -> UPDT_ID ) )
...
IF ( ( CRITERIA -> CARETEAM_CNT >0 ) )
 CALL PARSER (", DCP_PL_CUSTOM_ENTRY CE" )
IF ( ( CRITERIA -> BEST_ENCNTR_FLAG =0 ) AND ( ENCNTR_ORG_SEC_IND =1 ) )
SET  USEE  = 1
ENDIF
...
IF ( ( PATIENT_CNT >0 ) )
 CALL PARSER (
"and not expand (continueX, 1, patient_cnt, PPR.person_id, ptlstEncntr_reply->patients[continueX].person_id)" )
ENDIF
...
FROM ( PRSNL_GROUP_RELTN  PGR )
 PLAN ( PGR
WHERE  EXPAND ( Y , 1 ,  CRITERIA -> PROVIDER_GRP_CNT , PGR.PRSNL_GROUP_ID,  CRITERIA ->
 PROVIDER_GRPS [ Y ]-> GROUP_ID ) AND (PGR.PERSON_ID= CRITERIA -> RELTN_PRSNL_ID ) AND (
PGR.ACTIVE_IND=1 ))
 
 
HEAD PGR.PERSON_ID
 
IF ( (PGR.PERSON_ID= CRITERIA -> RELTN_PRSNL_ID ) )  PROVIDERFLAG =1
ENDIF
 
 WITH  NOCOUNTER
IF ( ( PROVIDERFLAG =0 ) )
SET  ptlstEncntr_reply -> STATUS_DATA -> STATUS  = "F"
SET  ptlstEncntr_reply -> SUBEVENTSTATUS [1 ]-> OPERATIONSTATUS  = "F"
SET  ptlstEncntr_reply -> SUBEVENTSTATUS [1 ]-> TARGETOBJECTVALUE  =
"Provider supplied does not belong to provider group"  GO TO  EXIT_SCRIPT
ENDIF
...
HEAD TEAM.PPR_CD
 PPR_CD_CNT =( PPR_CD_CNT +1 ),
IF ( ( MOD ( PPR_CD_CNT , 50 )=1 ) )  STAT = ALTERLIST ( RELTN -> RELTNS , ( PPR_CD_CNT +49 ))
ENDIF
...
FROM ( PRSNL_GROUP_RELTN  PGR )
 PLAN ( PGR
WHERE  EXPAND ( T , 1 ,  CRITERIA -> PROVIDER_GRP_CNT , PGR.PRSNL_GROUP_ID,  CRITERIA ->
 PROVIDER_GRPS [ T ]-> GROUP_ID ) AND (PGR.ACTIVE_IND=1 ))
 
 
HEAD PGR.PERSON_ID
 
IF ( (PGR.PERSON_ID= REQINFO -> UPDT_ID ) )  USERFLAG =1
ENDIF
...
SET  ptlstEncntr_reply -> SUBEVENTSTATUS [1 ]-> OPERATIONSTATUS  = "F"
SET  ptlstEncntr_reply -> SUBEVENTSTATUS [1 ]-> TARGETOBJECTVALUE  =
"User supplied does not belong to provider group"  GO TO  EXIT_SCRIPT
ENDIF
