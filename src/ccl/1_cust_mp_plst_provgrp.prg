...
DROP PROGRAM   1_cust_mp_plst_provgrp : DBA  GO
CREATE PROGRAM  1_cust_mp_plst_provgrp : DBA
...
DECLARE  EXECUTEVISITPRIMARYSELECT (( X = I4 )) =  NULL
 
DECLARE  EXECUTELIFEPRIMARYSELECT (( X = I4 )) =  NULL
...
IF ( ( CRITERIA -> PROVIDER_GRP_CNT =0 ) )
 CALL ECHO ("Definition Error: No provider group specified." ) GO TO  FINISH
...
SET  CRITERIA -> PROVIDER_GRPS [ IDX ]-> GROUP_ID  =  CRITERIA -> PROVIDER_GRPS [ CRITERIA ->
 PROVIDER_GRP_CNT ]-> GROUP_ID
...
WHERE  INITARRAY ( NSTART ,  EVALUATE (D1.SEQ, 1 , 1 , ( NSTART + NSIZE ))))
 AND ( PGR
WHERE  EXPAND ( IDX ,  NSTART , ( NSTART +( NSIZE -1 )), PGR.PRSNL_GROUP_ID,  CRITERIA ->
 PROVIDER_GRPS [ IDX ]-> GROUP_ID ,  NSIZE ) AND (PGR.ACTIVE_IND=1 ))
 AND ( TEAM
WHERE (TEAM.PRSNL_GROUP_RELTN_ID=PGR.PRSNL_GROUP_RELTN_ID))
 
 
HEAD REPORT
 PROV_RELTN -> LIFE_CNT =0 ,
 PROV_RELTN -> VISIT_CNT =0
DETAIL
 
IF ( (TEAM.PPR_FLAG=0 ) )  PROV_RELTN -> LIFE_CNT =( PROV_RELTN -> LIFE_CNT +1 ),
IF ( ( MOD ( PROV_RELTN -> LIFE_CNT , 10 )=1 ) )  STAT = ALTERLIST ( PROV_RELTN -> LIFE , (
 PROV_RELTN -> LIFE_CNT +9 ))
ENDIF
,  PROV_RELTN -> LIFE [ PROV_RELTN -> LIFE_CNT ]-> PPR_CD =TEAM.PPR_CD,  PROV_RELTN -> LIFE [
 PROV_RELTN -> LIFE_CNT ]-> PGR_ID =PGR.PERSON_ID
ELSEIF ( (TEAM.PPR_FLAG=1 ) )  PROV_RELTN -> VISIT_CNT =( PROV_RELTN -> VISIT_CNT +1 ),
IF ( ( MOD ( PROV_RELTN -> VISIT_CNT , 10 )=1 ) )  STAT = ALTERLIST ( PROV_RELTN -> VISIT , (
 PROV_RELTN -> VISIT_CNT +9 ))
ENDIF
,  PROV_RELTN -> VISIT [ PROV_RELTN -> VISIT_CNT ]-> PPR_CD =TEAM.PPR_CD,  PROV_RELTN -> VISIT [
 PROV_RELTN -> VISIT_CNT ]-> PGR_ID =PGR.PERSON_ID
ENDIF
 
FOOT REPORT
 STAT = ALTERLIST ( PROV_RELTN -> LIFE ,  PROV_RELTN -> LIFE_CNT ),
 STAT = ALTERLIST ( PROV_RELTN -> VISIT ,  PROV_RELTN -> VISIT_CNT )
 WITH  NOCOUNTER
SET  STAT  =  ALTERLIST ( CRITERIA -> PROVIDER_GRPS ,  CRITERIA -> PROVIDER_GRP_CNT )
SET  NSTART  = 1
SET  NSIZE  = 50
...
SET  PROV_RELTN -> VISIT [ IDX ]-> PGR_ID  =  PROV_RELTN -> VISIT [ PROV_RELTN -> VISIT_CNT ]->
 PGR_ID
SET  PROV_RELTN -> VISIT [ IDX ]-> PPR_CD  =  PROV_RELTN -> VISIT [ PROV_RELTN -> VISIT_CNT ]->
 PPR_CD
...
SUBROUTINE   EXECUTEVISITPRIMARYSELECT  ( X  )
...
ENDIF
 ...
 CALL PARSER ("    stat = alterlist(ptlstEncntr_reply->patients,patient_cnt+99)" )
 CALL PARSER ("  endif" )
 ...
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].filter_ind = encntr_org_sec_ind" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].encntr_id = epr.encntr_id" )
 ...
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].responsible_prsnl_id = epr.prsnl_person_id" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_cd = epr.encntr_prsnl_r_cd" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_id = epr.encntr_prsnl_reltn_id" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_flag = 1" )
 ...
 CALL PARSER ("    ptlstEncntr_reply->patients[patient_cnt].active_ind = 0" )
 CALL PARSER ("  endif" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].birthdate = p.birth_dt_tm" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].birth_tz = p.birth_tz" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].gender_cd = p.sex_cd" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].vip_cd = p.vip_cd" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].deceased_date = p.deceased_dt_tm" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].deceased_tz = p.deceased_tz" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].end_effective_dt_tm = e.end_effective_dt_tm" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].service_cd = e.med_service_cd" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].temp_location_cd = e.loc_temp_cd" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].visit_reason = e.reason_for_visit" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].visitor_status_cd = e.visitor_status_cd" )
 CALL PARSER ("  detail" )
 CALL PARSER ("  if ((epr.prsnl_person_id = reqinfo->updt_id" )
 CALL PARSER ("     and (epr.end_effective_dt_tm > cnvtdatetime(curdate, curtime3)" )
 CALL PARSER ("     or ptlstEncntr_reply->patients[patient_cnt].responsible_prsnl_id != reqinfo->updt_id))" )
 CALL PARSER ("     or (ptlstEncntr_reply->patients[patient_cnt].active_ind = 0" )
 CALL PARSER ("     and epr.end_effective_dt_tm > cnvtdatetime(curdate, curtime3)" )
 CALL PARSER ("     and ptlstEncntr_reply->patients[patient_cnt].responsible_prsnl_id != reqinfo->updt_id))" )
 CALL PARSER ("    ptlstEncntr_reply->patients[patient_cnt].responsible_prsnl_id = epr.prsnl_person_id" )
 CALL PARSER ("    ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_cd = epr.encntr_prsnl_r_cd" )
 CALL PARSER ("    ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_id = epr.encntr_prsnl_reltn_id" )
 CALL PARSER ("    ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_flag = 1" )
...
ENDIF
 ...
 CALL PARSER ("  if (ptlstEncntr_reply->patients[patient_cnt].responsible_prsnl_id = reqinfo->updt_id" )
 CALL PARSER ("    and ptlstEncntr_reply->patients[patient_cnt].active_ind = 1)" )
...
"    ptlstEncntr_reply->patients[patient_cnt].confid_level = uar_get_collation_seq(e.confid_level_cd)" )
 CALL PARSER ("  endif " )
 CALL PARSER ("  if (ptlstEncntr_reply->patients[patient_cnt].confid_level < 0)" )
...
SUBROUTINE   EXECUTELIFEPRIMARYSELECT  ( X  )
...
SET  NTOTAL  = ( CEIL (( CNVTREAL ( PROV_RELTN -> LIFE_CNT )/ NSIZE ))* NSIZE )
SET  STAT  =  ALTERLIST ( PROV_RELTN -> LIFE ,  NTOTAL )
IF (  (( ( CRITERIA -> LOCATIONS -> UNIT_CNT >0 ) )  OR  ( (( ( CRITERIA -> SERVICE_CNT >0 ) )  OR
 (( CRITERIA -> TYPE_CNT >0 ) ))  ))  )
SET  USEE  = 1
ENDIF
 
IF ( ( CRITERIA -> TIME_FRAME_FLAG != NOTIMEPARAMETER ) AND  USEE  )
SET  USEELH  = 1
ENDIF
 
IF ( ( USEE =0 ) AND  (( ( CRITERIA -> ADMIT_FLAG >0 ) )  OR  ( (( ( CRITERIA -> DISCHARGE_FLAG >0 )
 )  OR  (( CRITERIA -> LOOKFORWARD_ADMIT_FLAG >0 ) ))  ))  )
SET  USEE  = 1
ENDIF
...
ENDIF
 ...
 CALL PARSER ("idx = locateval (l, 1, patient_cnt, p.person_id, ptlstEncntr_reply->patients[l].person_id)" )
 ...
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].person_name = p.name_full_formatted" )
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].filter_ind = 0" )
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].encntr_id = 0" )
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].responsible_prsnl_id = ppr.prsnl_person_id" )
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_cd = ppr.person_prsnl_r_cd" )
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_id = ppr.person_prsnl_reltn_id" )
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].responsible_reltn_flag = 0" )
 CALL PARSER ("ptlstEncntr_reply->patients[patient_cnt].active_ind = 1" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].birthdate = p.birth_dt_tm" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].birth_tz = p.birth_tz" )
 CALL PARSER ("  ptlstEncntr_reply->patients[patient_cnt].gender_cd = p.sex_cd" )
...
IF ( ( CONFID_IND =1 ) )
 CALL PARSER (
"ptlstEncntr_reply->patients[patient_cnt].confid_level = maxval(0, uar_get_collation_seq(e.confid_level_cd))" )
ENDIF
...
ENDIF
 ...
 CALL PARSER ("detail" )
 CALL PARSER (
"if (idx = 0 and ptlstEncntr_reply->patients[patient_cnt].responsible_prsnl_id != reqinfo->updt_id)" )
...
HEAD P.PERSON_ID
 PRSNLCNT =( PRSNLCNT +1 ),
IF ( ( MOD ( PRSNLCNT , 50 )=1 ) )  STAT = ALTERLIST ( TEMP -> PERSONS , ( PRSNLCNT +49 ))
ENDIF
, TEMP -> PERSONS [ PRSNLCNT ]-> PERSON_ID =P.PERSON_ID, TEMP -> PERSONS [ PRSNLCNT ]-> NAME =
...
( DUMMYT  D2  WITH  SEQ = PATIENT_CNT )
 PLAN ( D1 )
 AND ( D2
WHERE ( ptlstEncntr_reply -> PATIENTS [D2.SEQ]-> RESPONSIBLE_PRSNL_ID = TEMP -> PERSONS [D1.SEQ]-> PERSON_ID ))
 
 
DETAIL
 ptlstEncntr_reply -> PATIENTS [D2.SEQ]-> RESPONSIBLE_PRSNL_NAME = TEMP -> PERSONS [D1.SEQ]-> NAME
 WITH  NOCOUNTER
ENDIF
...
IF ( ( CRITERIA -> TIME_FRAME_FLAG = NOTIMEPARAMETER ) )  RETURN (0 )
