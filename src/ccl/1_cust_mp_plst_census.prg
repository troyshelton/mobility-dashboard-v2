...
DROP PROGRAM   1_cust_mp_plst_census : DBA  GO
CREATE PROGRAM  1_cust_mp_plst_census : DBA
...
SET  ENCNTRREQUEST -> PERSONS [ X ]-> PERSON_ID  =  ptlstEncntr_reply -> PATIENTS [ X ]-> PERSON_ID
...
SET  STAT  =  ALTERLIST ( ENCNTRREQUEST -> PERSONS ,  ENCNTRCNT )
 EXECUTE PTS_GET_BEST_ENCNTR_LIST WITH  REPLACE ( ptlst_request ,  ENCNTRREQUEST ),
 REPLACE ( ptlstEncntr_reply ,  ENCNTRREPLY )
SET  ENCNTR_CNT  =  SIZE ( ENCNTRREPLY -> ENCOUNTERS , 5 )
FOR (  X  = 1  TO  ENCNTRCNT  )
 
FOR (  Y  = 1  TO  PATCOUNT  )
 
IF ( ( ptlstEncntr_reply -> PATIENTS [ Y ]-> PERSON_ID = ENCNTRREPLY -> ENCOUNTERS [ X ]-> PERSON_ID ) AND (
 ptlstEncntr_reply -> PATIENTS [ Y ]-> ENCNTR_ID =0 ) )
SET  ptlstEncntr_reply -> PATIENTS [ Y ]-> ENCNTR_ID  =  ENCNTRREPLY -> ENCOUNTERS [ X ]-> ENCNTR_ID
SET  Y  = ( PATCOUNT +1 )
ENDIF
...
DECLARE  EXECUTEPRIMARYSELECT (( X = I4 )) =  I2
...
IF ( ( CRITERIA -> LOCATIONS -> UNIT_CNT =0 ) AND ( CRITERIA -> SERVICE_CNT =0 ) )
...
IF ( ( CRITERIA -> LRELTN_CNT !=0 ) AND ( CRITERIA -> VRELTN_CNT =0 ) )
...
IF ( ( CONTINUE =0 ) )
 ...
 CALL PARSER ("stat = alterlist(ptlstEncntr_reply->patients, patient_cnt)" )
ENDIF
 
 CALL PARSER ("with nocounter go" ) RETURN ( CONTINUE )
 
 
END ;Subroutine
 
 
SUBROUTINE   PARSEENCNTRLOCHIST  ( X  )
...
ENDIF
 ...
 CALL PARSER ("and ELH.active_ind = 1" )
IF ( ( CRITERIA -> SERVICE_CNT >0 ) AND ( CRITERIA -> LOCATIONS -> UNIT_CNT >0 ) )
 CALL PARSER (
...
IF ( ( PATIENT_CNT >0 ) )
 CALL PARSER (
"and not expand (continueX, 1, patient_cnt, ELH.encntr_id, ptlstEncntr_reply->patients[continueX].encntr_id)" )
ENDIF
...
call echo('inside the pt census.prg')
