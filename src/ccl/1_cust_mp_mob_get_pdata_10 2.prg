...
JOIN dfc_pt
	...
	AND dfc_pt.component_cd = outerjoin(10891.00)  /* PRIMARY EVENT_ID */
...
JOIN c1_pt
	...
	;AND c1_pt.result_status_cd IN outerjoin(25, 34, 35)
...
JOIN c3_pt
	...
	AND c3_pt.event_cd = outerjoin(4348328.00)  /* Transfer Bed to and From Chair Rehab */
	AND c3_pt.valid_until_dt_tm > outerjoin(SYSDATE)
	;AND c3_pt.result_status_cd IN outerjoin(25, 34, 35)
	AND c3_pt.view_level = outerjoin(1)

ORDER BY c3_pt.encntr_id, dfa_pt.form_dt_tm DESC

HEAD c3_pt.encntr_id
	pt_hist_cnt = 0
...
DETAIL
	...
	drec->patients[d1.seq].pt_transfer_history[pt_hist_cnt].assist_level = c3_pt.result_val
	...
	; First entry (most recent) = current value for table
	if (pt_hist_cnt = 1)
		drec->patients[d1.seq].pt_transfer_assist = c3_pt.result_val
...
JOIN dfc_ot
	WHERE dfc_ot.dcp_forms_activity_id = outerjoin(dfa_ot.dcp_forms_activity_id)
	AND dfc_ot.component_cd = outerjoin(10891.00)  /* PRIMARY EVENT_ID */
	AND dfc_ot.parent_entity_name = outerjoin("CLINICAL_EVENT")

JOIN c1_ot
	WHERE c1_ot.event_id = outerjoin(dfc_ot.parent_entity_id)
	AND c1_ot.valid_until_dt_tm > outerjoin(SYSDATE)
	;AND c1_ot.result_status_cd IN outerjoin(25, 34, 35)
	AND c1_ot.view_level = outerjoin(1)

JOIN c2_ot
	WHERE c2_ot.parent_event_id = outerjoin(c1_ot.event_id)
	AND c2_ot.valid_until_dt_tm > outerjoin(SYSDATE)

JOIN c3_ot
	WHERE c3_ot.parent_event_id = outerjoin(c2_ot.event_id)
	AND c3_ot.event_cd = outerjoin(4348328.00)  /* Transfer Bed to and From Chair Rehab */
	AND c3_ot.valid_until_dt_tm > outerjoin(SYSDATE)
	;AND c3_ot.result_status_cd IN outerjoin(25, 34, 35)
	AND c3_ot.view_level = outerjoin(1)

ORDER BY c3_ot.encntr_id, dfa_ot.form_dt_tm DESC

HEAD c3_ot.encntr_id
	ot_hist_cnt = 0
...
DETAIL
	...
	drec->patients[d1.seq].ot_transfer_history[ot_hist_cnt].assist_level = c3_ot.result_val
	...
	; First entry (most recent) = current value for table
	if (ot_hist_cnt = 1)
		drec->patients[d1.seq].ot_transfer_assist = c3_ot.result_val
